/**
 * Created by luckybai on 2017/3/10.
 */
//命名规范:
//    class = 'class-name';
//    id = 'id-name';
...
$$.prototype = {
    ...
    //添加IP和位置检测；生成的盒子，id="ipLocation";
    ipLocation: function(id) {
        var script = document.createElement('script');
        var url = 'http://pv.sohu.com/cityjson';
        script.src = url;
        document.body.appendChild(script);
        var div = document.createElement('div');
        div.setAttribute('id', 'ip_location');
        // IE6/7/8支持onreadystatechange事件
        // IE9/10支持onreadystatechange和onload事件
        // Firefox/Chrome/Opera支持onload事件
        //因为不是返回的函数如：jsonp({})，所以没法等返回时候调用自定义同名函数，只能监测
        if (script.onreadystatechange) {
            script.onreadystatechange = function () {
                div.innerHTML = '您的IP地址：' + returnCitySN.cip + ' 您所在的位置：' + returnCitySN.cname;
